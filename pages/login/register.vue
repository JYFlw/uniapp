<template>
	<view>
		...
		<view class="form re">
			<view class="password">
				<input placeholder="姓名" v-model="name" password=true placeholder-style="color: rgba(255,255,255,0.8);" />
			</view>
			<view class="username">
				<view class="get-code" :style="{'color':getCodeBtnColor}" @click.stop="getCode()">{{getCodeText}}</view>
				<input placeholder="工作单位" v-model="workplace" placeholder-style="color: rgba(255,255,255,0.8);" />
			</view>
			<view class="username">
				<view class="get-code" :style="{'color':getCodeBtnColor}" @click.stop="getCode()">{{getCodeText}}</view>
				<input placeholder="职位" v-model="post" placeholder-style="color: rgba(255,255,255,0.8);" />
			</view>
			<!-- <view class="code">
	...
	export default {
		...
		methods: {
			...
			doReg() {
				//发起post方式的请求
				uni.request({
					url: "http://192.168.18.194:8888/api/register", //仅为示例，并非真实接口地址。
					data: {
						username: this.name,
